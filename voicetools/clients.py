# coding: utf-8
import requests
from .constants import BaiduUrl, EmotibotUrl, ErrNo
...
class BaseClient(object):
    ...
    def post_request(self, url, data, params=None, headers=None, connect_timeout=None, request_timeout=None):
...
class EmotibotClient(BaseClient):

    """Client for handling the process of Turing requests."""

    def __init__(self, **kwargs):
        super(EmotibotClient, self).__init__(**kwargs)

    def query(self, params):
        try:
            resp = self.get_request(url=EmotibotUrl.emotibot_url, params=params)
        except Exception, e:
            raise e
        return resp.json()

emotibotclient = EmotibotClient()
